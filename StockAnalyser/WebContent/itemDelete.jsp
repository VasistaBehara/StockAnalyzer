...
<body onload="history.forward(1)">
	<ul class="sidenav">
		...
		<li><a class="active" href="itemDelete.jsp">Delete items</a></li>
	...
	<div class="content">
 	...
 	try {
 		...
 		ResultSet rs = st.executeQuery("Select itemName from item");
 %>
				<h1 align="center">
					<u>Delete item</u>
				</h1>
				<form align="center" action="itemDelete.jsp">
					<select name="items">
						<option>select item to delete</option>
						<%
							while (rs.next()) {
									String itemName = rs.getString("itemName");
						%>
						<option value=<%out.println(itemName);%>>
							<%
 	...
 	try {
 		...
 		con1 = DriverManager.getConnection("jdbc:mysql://localhost:3306/stockanalyser", "root", "2513218");
 		st = con1.createStatement();
 		String query = "delete from item where itemName='" + item + "'";
 		System.out.println(query);
 		int i = st.executeUpdate(query);
 		if (i > 0)
 			out.println("Successfully deleted item from database");
 		else
 			out.println("");
 	} catch (Exception e) {
 		System.out.print(e);
